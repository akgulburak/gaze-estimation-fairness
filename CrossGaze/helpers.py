...
def get_trainer(args, train_loader, device, optimizer, scheduler, criterion, depth_encoder=None, depth_decoder=None):
	if args.trainer == 'simple':
		...
		if args.use_class_weights:
			trainer = PartTrainerClassWeights(args, train_loader, device, optimizer, scheduler, criterion)
		else:
...
def get_loaders(args, train_transforms, test_transforms, label_name="train.label", test_label_name="test.label", use_reweight=False, use_oversample=False, use_class_weights=False):
	image_dir = os.path.join(args.data_dir, 'Image/')
	train_label_file = os.path.join(args.data_dir, os.path.join('Label', label_name))
	val_label_file = os.path.join(args.data_dir, os.path.join('Label', test_label_name))

	train_dataset = CustomDatasetDemographicGroups(train_label_file, image_dir, train_transforms, args.angle, args, 
												   train=True, use_reweight=use_reweight, use_oversample=use_oversample, 
