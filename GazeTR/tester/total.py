...
def main(train, test, args):
    ...
    # dataframe = pd.read_csv(args.label)
    ...
    net.cuda(); net.load_state_dict(torch.load(args.pretrained_model)); net.eval()
            ...
            for j, (data, label) in enumerate(dataset):
                ...
                for k, gaze in enumerate(gazes):
                    ...
                    if False:
                        ...
                        selected_person = dataframe[dataframe["id"]==person_id]
                        ethnicity = selected_person["group"].item()
                    else:
                        demographic_label = data['demographic_label'][k]

                    row = [gaze, gt, error, demographic_label]
...
if __name__ == "__main__":
    ...
    main(train_conf.train, test_conf.test, args)
