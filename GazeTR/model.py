...
class Model(nn.Module):
    ...
    def apply_class_weighted_loss(self, predicted, groundtruth, loss_function, weight):
    ...
    def loss(self, x_in, label, use_class_weighted_loss, weight=None):
        if use_class_weighted_loss:
            gaze = self.forward(x_in)
            loss = self.apply_class_weighted_loss(gaze, label, self.loss_op, weight)
            return gaze, loss
        else:
            gaze = self.forward(x_in)
