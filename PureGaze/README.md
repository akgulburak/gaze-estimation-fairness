...
2. modifing files in `config/` folder, and run commands like: 
    ...
    **Training:**`uv run python trainer/trainer.py -c config/train/config-gaze360.yaml`

    **Test:**`uv run python tester/total.py -s config/train/config-gaze360.yaml -t config/test/config-gaze360.yaml -o <csv_output_file_name> -m <trained_model_path>`
