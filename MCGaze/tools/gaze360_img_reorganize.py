...
################### you should modify the path to raw gaze360 dataset  ############################
ori_dataset_root = "C:/Gaze/Gaze360/imgs"
meta_data_root = "C:/Gaze/Gaze360/metadata.mat"
origt = "C:/Gaze/Gaze360"
###################################################################################################
...
dataset_settings = ['L2CS']
target_dataset_roots = ['data/l2cs/']
splits = ['train','test']

recording_person_identity_pairs = {}

for dataset_setting, target_dataset_root in zip(dataset_settings, target_dataset_roots):
    for split in splits:
        output_file = split + "_paths.csv"
        with open(output_file, mode="w", newline="", encoding="utf-8") as file:
            writer = csv.writer(file)
            writer.writerow(["id", "new_image_path"])
            ...
            for ori_gt_info in tqdm(ori_gt_infos):
                ...
                if dataset_setting =="L2CS":
                    if (face_bbox[point_index] != np.array([-1, -1, -1, -1])).all():
                        ...
                        splitted_path = ori_img_path.split("/")

                        recording_id = splitted_path[-4].replace("rec_", "").lstrip("0")
                        person_identity_id = splitted_path[-2].lstrip("0")
                        if len(person_identity_id)==0:
                            person_identity_id="0"
                        person_pair = str(recording_id) + "_" + str(person_identity_id)
                        writer.writerow([person_pair, new_img_path])
                    ...
                    splitted_path = ori_img_path.split(os.sep)
                    ...
                    if len(person_identity_id)==0:
                        person_identity_id="0"
                    person_pair = str(recording_id) + "_" + str(person_identity_id)
                    # if person_pair not in recording_person_identity_pairs:
                    #     recording_person_identity_pairs[person_pair] = []

                    cv2.imwrite(new_img_path, cur_img)

                    # recording_person_identity_pairs[person_pair].append(new_img_path)
                    writer.writerow([person_pair, new_img_path])
