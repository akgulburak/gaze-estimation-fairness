import sys
sys.path.insert(0, "/home/ubuntu/vol/MCGaze/MCGaze/MCGaze-master")
...
def main(args):
    ...
    for video in tqdm(anno['videos']):
        ...
        for clip_index in range(0, clip_num):
            ...
            datas = []
            for img in cur_clip:
                img_path = os.path.join(args.root, "test", "Face", img)
                if not os.path.isfile(img_path):
                    continue
                data = dict(img_info=dict(filename=img), img_prefix=os.path.join(args.root, "test", "Face"))
            ...
            #print(datas['img_metas'][0][0])
            #import torchvision
            #print(datas['img'][0][0].cpu().numpy().shape)
            #a = torch.permute(datas['img'][0][0], [1, 2, 0])
            #torchvision.utils.save_image(datas['img'][0][0]+1, "testetst.png")
            #exit()
