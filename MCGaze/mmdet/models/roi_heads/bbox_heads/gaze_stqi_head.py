...
class GazeSTQIHead(BBoxHead):
    ...
    def loss(self,
        ...
        losses['face'] = self.head_loss(cls_score[0], bbox_pred[0], labels[0], label_weights[0], bbox_targets[0], bbox_weights[0], imgs_whwh[0], all_weights=all_weights)
        losses['eyes'] = self.head_loss(cls_score[1], bbox_pred[1], labels[1], label_weights[1], bbox_targets[1], bbox_weights[1], imgs_whwh[1], all_weights=all_weights)
        losses['head'] = self.head_loss(cls_score[2], bbox_pred[2], labels[2], label_weights[2], bbox_targets[2], bbox_weights[2], imgs_whwh[2], all_weights=all_weights)
        return losses
    ...
    def head_loss(self,
        ...
        if cls_score is not None:
            if cls_score.numel() > 0:
                ...
                #    label_weights = torch.tensor(all_weights, device="cuda:0").flatten()
