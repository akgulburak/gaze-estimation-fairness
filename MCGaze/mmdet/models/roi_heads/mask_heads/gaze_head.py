...
class GazeHead(BaseModule):
    ...
    def loss(self, gaze_results, gaze_targets, gaze_weights, reduction_override=None, all_weights=None):
        loss = dict()
        ...
        loss['final_gaze'] = self.gaze_loss(gaze_results['gaze_score'], gaze_targets[2], gaze_weights[2], using_arccos=True, usingtemp=True, all_weights=all_weights)
        loss['face_gaze'] = self.gaze_loss(gaze_results['face_gaze_score'], gaze_targets[0], gaze_weights[0], using_arccos=True, all_weights=all_weights)
        loss['eyes_gaze'] = self.gaze_loss(gaze_results['eyes_gaze_score'], gaze_targets[1], gaze_weights[1], using_arccos=True, all_weights=all_weights)
        loss['head_gaze'] = self.gaze_loss(gaze_results['head_gaze_score'], gaze_targets[2], gaze_weights[2], using_arccos=True, all_weights=all_weights)
        
        return loss


    @force_fp32(apply_to=('gaze_pred', ))
    def gaze_loss(self, gaze_pred, gaze_targets, gaze_weights, using_arccos=False, usingtemp=False, reduction_override=None, all_weights=None):
        ...
        if self.loss_gaze != None and using_arccos:
            if all_weights is not None:
                all_weights = torch.tensor(all_weights, device="cuda:0").flatten()
                loss_gaze = self.loss_gaze(
                    ...
                    avg_factor=avg_factor, all_weights=all_weights)
            else:
                loss_gaze = self.loss_gaze(
                    gaze_pred[pos_inds.type(torch.bool)],
                    gaze_targets[pos_inds.type(torch.bool)],
