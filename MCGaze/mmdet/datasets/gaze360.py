...
def mixup_data(x, y, alpha=1.0, use_cuda=True, clip=0.05):
    ...
    permute_y = y
    if lam < 0.5:
        permute_y = DC(torch.stack(y.data, axis=0)[index])
    return mixed_x, permute_y, lam, permute_index
...
class Gaze360Dataset(CustomDataset):
    ...
    def balance_blocks(self, a, b, seed=None):
        rng = random.Random(seed)
        id_to_block = defaultdict(list)
        for id_, second in a:
            id_to_block[id_].append((id_, second))

        id_to_group = {}
        for (ids, row) in b.iterrows():
            try:
                id_to_group[row["video_ids"]] = row["demographic_groups"]
            except:
                print(row)
        group_ids = [sublist for sublist in id_to_block]
        group_counts = {}
        ...
        group_to_ids_present = defaultdict(list)

        for id_ in id_to_block.keys():
            grp = id_to_group.get(id_)
            if grp is not None:
                group_to_ids_present[grp].append(id_)

        target = max(len(v) for v in group_to_ids_present.values())

        out = list(a)
        for grp, ids_present in group_to_ids_present.items():
            need = target - len(ids_present)
            if need <= 0:
                continue
            if not ids_present:
                continue

            picks = [rng.choice(ids_present) for _ in range(need)]
            for pid in picks:
                out.extend(id_to_block[pid])

        return out

    def get_demographic_groups(self):
        label_dataframe = pd.read_csv(self.labels_file_path)

        label_dataframe['video_ids'] = label_dataframe['new_image_path'].str.split("/").str[3]
        label_dataframe['video_ids'] = label_dataframe['video_ids'].astype(int)-1

        #reduced = label_dataframe['video_ids'].unique().tolist()
        
        #result = label_dataframe.groupby('ethnicities')['video_ids'].apply(list).to_dict()
        demographic_groups = label_dataframe[["demographic_groups", "video_ids"]]

        demographic_group_result = (
            demographic_groups
            .apply(lambda x: x)  # make values unique per group
            .to_dict()
        )

        return demographic_group_result, demographic_groups, label_dataframe

    def apply_oversample(self, data_infos, group_dict):
        data_infos = self.balance_blocks(data_infos, group_dict)
        return data_infos
    ...
    def __init__(self,
        ...
        self.demographic_group_dict, self.demographic_group_labels, self.label_dataframe = self.get_demographic_groups()

        self.weights = self.calculate_weights(self.demographic_group_dict)

        if use_oversample:
            self.data_infos = self.apply_oversample(self.data_infos, self.demographic_group_labels)
            print("Using oversample")
        else:
            print("Not using oversample")
...
# 此处的数据类型还都不是tensor
    ...
    def calculate_weights(self, groups_dict):
        weights = []
        demographic_groups_dict = groups_dict["demographic_groups"]
        number_of_total_samples = len(demographic_groups_dict)
        for group_key in demographic_groups_dict.values():
            if group_key == "other":
                group_weight = 1
            else:
                group_weight = number_of_total_samples / list(demographic_groups_dict.values()).count(group_key)
...
# prepare_train_clip -> prepare_train_img -> get_ann_info -> _parse_ann_info
    def __getitem__(self, idx):
        ...
        while True:
            ...
            data = self.prepare_train_clip(idx, batch_weights)
            if data is None:
                idx = self._rand_another(idx)
                continue
            return data

    def prepare_train_clip(self, idx, batch_weights=None):
        vid, frame_id = self.data_infos[idx]
        vid_info = self.vid_infos[vid]
