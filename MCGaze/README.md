...
Here is the right hierarchy of folder `MCGaze/data` below:
 ```
  └── data
      ...
      └──
  ``````
### Prepare GazeCapture dataset
The GazeCapture dataset is converted to a similar format with the script ```Gazecapture/test_dataset_prepare_mcgaze.py```. Set the paths correctly and use this script to use the GazeCapture dataset for evaluation.
...
If you use uv package manager, run the code with:

```
uv run python tools/test_gaze360_gaze.py configs/multiclue_gaze/multiclue_gaze_r50_l2cs.py <trained_checkpoint_file> --output_file <output_json_file_name> --json <gazecapture_dataset_json_input> --root <image_folder_path>
```

### Training

* Run the commands below to begin training in different settings. Change the configs to run with a different bias mitigation method.
