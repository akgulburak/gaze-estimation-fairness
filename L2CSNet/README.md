...
You can add one of the following flags to train with a bias mitigation method:
```
    [--oversample, --class-weight, --reweight]
```
...
```
 ...
 --gpu 0 \
 --gaze360image_dir <image_directory_path> \
 --gaze360label_dir <label_file_path> \
 --output <output_csv_file_name>
```
The model predictions are created in a file given with the ```--output``` parameter.

Use ```--dataset gaze360``` for GazeCapture too, as it has the same dataset structure with Gaze360.