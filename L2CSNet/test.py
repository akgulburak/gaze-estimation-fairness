...
def auto_gamma_batch(x, gamma_min=0.2, gamma_max=1.0):
    """
    ...
    mean = gray.mean(dim=(1, 2))  # [B]

    # Map brightness → gamma
    gamma = torch.clamp(1.5 - mean, gamma_min, gamma_max)  # [B]
        ...
        with open(os.path.join(evalpath,data_set+".log"), 'w') as outfile:
            with open(args.output, "w", newline="") as output_file:
                ...
                if True:
                    ...
                    count = 0
                    with torch.no_grad():
                        for j, (images, labels, cont_labels, name, weight, K, demographic_labels) in tqdm(enumerate(test_loader)):
                            ...
                            for k in range(len(pitch_predicted)):
#                            for p,y,pl,yl in zip(pitch_predicted,yaw_predicted,label_pitch,label_yaw):
                                p,y,pl,yl = pitch_predicted[k],yaw_predicted[k],label_pitch[k],label_yaw[k]
                                error = angular(gazeto3d([p,y]), gazeto3d([pl,yl])).item()
                                avg_error += error

                                demographic_label = demographic_labels[k]
                                result = [pitch_predicted[k].item(),yaw_predicted[k].item()]
                                gt = [label_pitch[k].item(),label_yaw[k].item()]
